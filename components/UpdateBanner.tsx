...
export function UpdateBanner({ state, progress=0, onRestart, onDismiss, downloadUrl }: { state: "downloading"|"ready"|"blocking"; progress?: number; onRestart?: ()=>void; onDismiss?: ()=>void; downloadUrl?: string }) {
  ...
  if (state==="downloading") {
    return (
      <View style={{ paddingTop: insets.top }} className="w-full bg-surface dark:bg-surface-dark px-4 py-3 gap-2 border-b border-border dark:border-border-dark">
  ...
  return (
    <View style={{ paddingTop: insets.top }} className="w-full bg-surface dark:bg-surface-dark px-4 py-3 flex-row items-center justify-between border-b border-border dark:border-border-dark">
