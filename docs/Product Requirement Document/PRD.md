...
> Last updated: 2026-08-30 (P0-1: Balance reconciliation + P0-2: Extended search + P0-3: Decimal hint — 3 polishes)
...
- **Budgets** (when budgets exist): up to 3 budget pills showing category name,
  spent/budgeted amounts, and a color-threshold progress bar (`C.success` → `C.chartAmber` → `C.error`, `useThemeColors()`-aware: `#D97706` light / `#F59E0B` dark for amber, via `app/(tabs)/home.tsx:88` `BudgetPill`). "See All" links to the Budgets tab. When no budgets exist, the empty
  state's "Create Budget" action is available to Owners and Members alike.
- **Analytics (below Budgets, as of 2026-08-31 — fixes 2026-08-31):**
  - **Delta Card** (`components/charts/DeltaCard.tsx`): `GradientCard` showing `currentNet` (this month) vs `prevNet` (last month) — looked up by `periodStart === monthStart/prevMonthStart` (not `cashflow[4]/[5]` index) via `calcDelta` (`utils/analytics.ts`); `prevNet===0` → "New this month" / "No change"; **no chip/shape** — right side is plain `Feather trending-up/down/minus` + `Text` `label` with `color C.success/C.error/C.textSecondary` and `withSpring` scale only (no `backgroundColor`/`borderColor`/`borderRadius`/`Shadow.card`/`Radius.md`), so it blends with `GradientCard` warm bg (`#FFFBF5`→`#FEF3C7`) without a pill. `transactions.cashflow` takes optional `timezone` (client `resolveTimezone(household.timezone)`).
