...
- Forms use `KeyboardAwareScrollView` (`react-native-keyboard-controller`)
  instead of RN `KeyboardAvoidingView` — the latter is inert on Android
  (`behavior` was iOS-only) and let edge-to-edge keyboards cover bottom fields.
  Do not reintroduce `KeyboardAvoidingView` on form screens.
