...
> Last updated: 2026-08-31 (Period handling — swipeable Home, household periodType/balanceMode, periodBalances snapshot (fresh/carryOver), extensible to weekly/yearly (B))
...
| Household | Create (first user becomes Owner), get active household, rename (Owner only), list members, remove member (Owner only; owner cannot be removed). Preference `periodType` (`monthly` default, extensible `weekly`/`yearly` — `weekly`/`yearly` coming soon via `households.updatePeriodType`) and `balanceMode` (`fresh` default vs `carryOver`) — Owner-only mutations `households.updateBalanceMode`/`updatePeriodType` (validated via `validateBalanceMode`/`validatePeriodType`, patch `households` + recompute cascade `periodBalances`). |
...
| Home | Dashboard (swipeable period — `PagerView` 12 periods, `<`/`>` + dots + `formatPeriodLabel` from `utils/period.ts`, `selectedPeriodStart` binding): household card, Total Balance (with period net `periodBalances.get`), Budgets (3 pills per `selectedPeriodStart`), **Analytics (below Budgets): Delta closing vs prev closing (`periodBalances` `closingBalance` delta, `calcDelta`) + Cashflow 6-period bar chart from `periodBalances.listWindow` (income Success / expense Error) + Spending by Category donut (`spendingByCategory` this period, legend + center total)** — all household-timezone-aware via `getPeriodBounds` (`monthly`/`weekly`/`yearly`), Member hidden-category excluded (periodBalances snapshot O(1) read, spending single-scan), pure-View + reanimated with Pressable tooltip; My Accounts, Recent Transactions (per `selectedPeriodStart`, paginated, grouped by day, "See All"). PTR & stale banner still applies. |
| Analytics | Cashflow 6 periods + Spending by Category (selected period) + Delta closing vs prev closing on Home below Budgets. Period snapshot `periodBalances.listWindow` (6-period O(1) snapshot read, no scan) + `spendingByCategory` (1 `by_household_date` scan per query, hidden-category cache, window validation). Delta uses `periodBalances.closingBalance` vs `prevClosing` (`balanceMode` aware: `fresh` closing=net, `carryOver` cumulative). |
...
- **Period navigation (as of 2026-08-31 — swipeable):** header outside `PagerView` with `<` `>` chevrons (44×44, `Shadow.card`, `Radius.md`, `C.border`, disabled at current period `next > curStart`) + centered `formatPeriodLabel(selectedPeriodStart, tz, periodType)` (`utils/period.ts`) + dots (12 periods via `buildPeriodWindow(now, tz, periodType, 12)`, active `w 16 C.primary` vs `w 6 C.border`). Swipeable `react-native-pager-view` `PagerView` (paging, `initialPage selectedIndex`, `onPageSelected` → `setSelectedPeriodStart` + `hapticSuccess`), each page is a `ScrollView` with `RefreshControl` (`refreshKey` + `hapticSuccess`). All sections bind to `selectedPeriodStart`: `periodEnd = getPeriodBounds(selectedPeriodStart, tz, periodType).end`. `periodType = household.periodType ?? "monthly"`, `timezone = resolveTimezone(household.timezone)`. Extensible `periodType` `monthly`/`weekly`/`yearly` (weekly Monday 00:00, yearly Jan 1 00:00) via `utils/period.ts` (B).
...
- **Analytics (below Budgets, as of 2026-08-31 — fixes 2026-08-31 + period snapshot 2026-08-31):**
  - **Delta Card** (`components/charts/DeltaCard.tsx`): `GradientCard` showing **closing delta** `currentClosing` ( `balances.closingBalance` for `selectedPeriodStart`) vs `prevClosing` (`prevBalances.closingBalance`) — derived from `periodBalances.get` (not `transactions.cashflow` scan) via `calcDelta` (`utils/analytics.ts`); `prevClosing===0` → "New this month" / "No change"; **no chip/shape** — right side is plain `Feather trending-up/down/minus` + `Text` `label` with `color C.success/C.error/C.textSecondary` and `withSpring` scale only (no `backgroundColor`/`borderColor`/`borderRadius`/`Shadow.card`/`Radius.md`), so it blends with `GradientCard` warm bg (`#FFFBF5`→`#FEF3C7`) without a pill. In `fresh` mode `closing = net`, in `carryOver` `closing` is cumulative — delta reflects `balanceMode`.
  - **Cashflow 6-Period Bar Chart** (`components/charts/CashflowBarChart.tsx`): 6 period groups (household timezone via `buildPeriodWindow(now,tz,periodType,6)` or `analyticsWindow`, extensible periodType) from `periodBalances.listWindow` (`{startDate,endDate,periodType}` O(1) snapshot, not scan, `balances[]` → `periodStart/label/income/expense/net`), paired bars (Income `success` / Expense `error`, max 100px, scale = `maxBarValue`), extracted `AnimatedBar` (avoids hook-in-loop) with reanimated `withDelay(i*60,withTiming(400))`, `Pressable` tooltip per period (`Income +X, Expense -Y, Net Z` + `hapticSuccess`, `Shadow.elevated`, no `style` callback); empty → "No transactions in last 6 months".
  - **Spending by Category Donut** (`components/charts/SpendingDonut.tsx` + `react-native-svg 15.12.1`): Selected period expense only (`api.transactions.spendingByCategory` `{startDate: selectedPeriodStart, endDate: periodEnd}` per period, or retain month scope if weekly/yearly but currently `monthly`), top 5 + aggregated "Others" (`overflowAmount`), palette `C.accountCash/Bank/Ewallet/CreditCard/primary` + 2 accents, **colored donut via `react-native-svg` `Circle` `strokeDasharray` (r=15.915, circumference 100, `dash=pct*100`, `offset=25-cumulative*100`)** over track `C.border` (r 15.915, w 7), inner cutout 80px `C.background` showing `formatNumber(total)`; legend `FadeIn.delay(i*40)` + `Pressable` selectable row (`% • amount` when selected, `hapticSuccess`, `isDimmed` `opacity 0.35` on non-selected arcs, `strokeWidth 8.5` when selected); `Others` row shows `+N more` → `%` when selected; empty → card "No spending this month". Requires `npx expo install react-native-svg` (included in Expo Go 54, still OTA-eligible).
  - All cards use `Shadow.card`, `Radius.md`, `useThemeColors()` (dark variants), `Feather` icons. Loading: `Skeleton` 80/180/140. Hidden category excluded for Members (periodBalances snapshot excludes transfers, aggregates income/expense signed; spending cached `hiddenCategoryCache`). Offline: shows cached Convex data + `ConnectivityBanner`.
- **My Accounts**: horizontal account cards with type-specific tinted icon
  ...
  in the Accounts tab).
- **Recent Transactions**: latest 5 per `selectedPeriodStart` (`useQuery(api.transactions.list, {startDate: selectedPeriodStart, endDate: periodEnd, limit:5})` per period), grouped by day with day net total (income − expense; transfers excluded), "See All" link to the Transactions tab. Transaction icons map category names to relevant Feather icons (shopping-cart, coffee, car, home, briefcase, etc.) with semantic colors (green for income, red for expense).
- **PTR & stale real (as of 2026-08-28):** 4 tabs use hooks/useConnectivity (NetInfo) — banner appears instantly when isConnected===false, fallback undefined >3s otherwise; RefreshControl and banner Retry show a 600ms spinner, clear stale state, and trigger haptic, relying on Convex reactive subscription for fresh data (no manual cache invalidation). Requires native rebuild for NetInfo. Home pager keeps `selectedIndex = pagerPeriods.findIndex(p.periodStart===selectedPeriodStart) ?? last` and `stale` derives from `balances === undefined` (period snapshot) + analytics `cashflowRes === undefined || spendingRes === undefined`.
...
- **Balance modes:** `fresh` (default): `opening 0`, `closing = income-expense` isolated per period; `carryOver`: `opening = prev closing`, `closing = opening + net` cumulative cascade (sorted `periodStart` from `household.createdAt` to now, guard 500 periods, `computeOpeningClosing`). Verified by `tests/periodBalances.test.ts:1` (fresh vs carryOver 2 periods, owner gate).
- **Recompute cascade:** `convex/periodBalances.ts:114` `recomputeAllForHousehold(ctx, household, now)` — single `by_household_date` scan `gte household.createdAt lt now` `take(10001)` (cap 10k, else `Too many transactions`), group by `getPeriodBounds(tx.date, timezone, periodType).start` (income sum, expense sum `Math.abs`), build ordered `firstStart..currentStart` via `getNextPeriod` (guard 500), compute opening/closing, upsert (`by_household_period` unique). `recomputeFromForHousehold` delegates to full recompute for `carryOver` correctness. Triggers: `households.updateBalanceMode`/`updatePeriodType` after patch, `transactions.create/update/delete` (`convex/transactions.ts:135`, 9xx) `recomputeFrom(ctx, householdId, date)`, `accounts.create` (`convex/accounts.ts:36`) after opening transaction, `periodBalances.recomputeAll`/`reconcile`/`backfill` owner mutations. `verify` (query) compares stored vs expected and returns `{discrepancies, isOwner}`.
- **Queries:** `periodBalances.get({periodStart, periodType?})` → snapshot or null (member-safe via `findUserAndMembership`); `periodBalances.listWindow({startDate,endDate,periodType?})` → `{balances, isOwner}` filtered `periodStart >= startDate && < endDate` sorted; `verify`/`reconcile`/`backfill`/`recomputeAll` as above. `utils/period.ts` `validatePeriodType`/`validateBalanceMode` guard inputs; `constants/validation.ts` `PERIOD_TYPES`/`BALANCE_MODES` shared client/server.
- **Client:** Home `selectedPeriodStart` (see §3.8) reads `periodBalances.get` for period net/closing + `prevBalances` for delta, `listWindow` for cashflow 6-period window (`analyticsWindow` via `buildPeriodWindow`), `spendingByCategory` per period for donut; Settings segmented control toggles `balanceMode` (see §3.2). Weekly/yearly `getWeekBounds` (Monday 00:00) / `getYearBounds` (Jan 1 00:00) already implemented in `utils/period.ts` and wired through `getPeriodBounds` switch, blocked server-side until GA (`Weekly/yearly coming soon`).
...
| `app/(tabs)/home.tsx` | Swipeable dashboard (`react-native-pager-view` paging, 12 periods, `selectedPeriodStart` via `getPeriodBounds`/`buildPeriodWindow`, `<`/`>` + dots `formatPeriodLabel`, `pagerRef` + `hapticSuccess`): household card, Total Balance (period net via `periodBalances.get`), budget pills per `selectedPeriodStart` (+ `periodEnd`), Analytics below Budgets — `DeltaCard` (closing delta via `periodBalances`), `CashflowBarChart` (6 periods via `listWindow`), `SpendingDonut` (selected period via `spendingByCategory`); `BudgetPill` is `memo`, `PagerView` + `ScrollView` `RefreshControl` (`refreshKey`), stale derives from `balances === undefined` + analytics |
| `components/charts/*` | `DeltaCard` (GradientCard, `closingBalance` delta `currentClosing` vs `prevClosing` via `calcDelta`, `withSpring` scale, `Feather` trending icon, no chip), `CashflowBarChart` (`periodBalances.listWindow` data `periodStart/label/income/expense/net`, `AnimatedBar` `withDelay` stagger, tooltip), `SpendingDonut` (`react-native-svg` `Circle` `strokeDasharray` colored arcs, inner cutout, `FadeIn` + selectable highlight) — all `react-native-reanimated` + `Pressable` tooltip + `useThemeColors()` |
| `utils/period.ts` | Period abstraction (`PeriodType` `monthly`\|`weekly`\|`yearly`, `BalanceMode` `fresh`\|`carryOver`, `getWeekBounds` Monday 00:00 tz, `getYearBounds` Jan 1 00:00 tz, `getPeriodBounds(ts,tz,type)` switch, `getPrevPeriod`/`getNextPeriod`/`formatPeriodLabel`/`buildPeriodWindow`, wraps `utils/date.ts` `getMonthBounds`; `validatePeriodType`/`validateBalanceMode` + `PERIOD_TYPES`/`BALANCE_MODES` mirror `constants/validation.ts`) |
| `utils/analytics.ts` | `buildSixMonthWindow(now, timezone)` (cur 6× `getMonthBounds`, labels `formatMonthLabel`), `calcDelta(currentNet/CLOSING, prevNet/CLOSING)` (`null` when `prev 0` → "New this month"/"No change", else `+X.X% vs last month`), `maxBarValue(data)` — now fed `periodBalances.closingBalance` for delta |
