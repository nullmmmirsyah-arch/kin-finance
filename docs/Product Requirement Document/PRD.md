...
> Last updated: 2026-09-06 (Category Visible/Hidden sections + responsive IconPicker — filter All/Income/Expense partitioned into Visible/Hidden, badge -/+/x, bulk delete hidden, responsive 4-6 cols)
...
reference the category. **Categories page (`app/categories.tsx`) partitions the filtered list (`All`/`Income`/`Expense` Chips retained + Add button `C.primary`) into Visible (`hidden=false`) and Hidden (`hidden=true`) vertical sections** (In Use / Not Used reference): Visible header `Visible (N)` + `(Long press to reorder)` placeholder (Phase 1 no drag), Hidden header `Hidden (N)` + owner-only `Delete hidden categories` underline link (`Alert` confirm → bulk `remove` sequential `Promise.allSettled` with `Snackbar` summary); Visible cards badge `-` yellow `#FACC15` top-left to hide (`update hidden:true`), Hidden cards badge `+` yellow top-left to unhide + `x` red `#FCA5A5` top-right to delete (18px circles, `border 2 C.cardBorder`, `hitSlop 6`, `Feather` 10px). `category-form` IconPicker responsive fixes 4-col stretched screenshot (HP 4-col wasted right padding → 5/6 cols on 360/390dp, no horizontal scroll).
...
|------|------|-------------|
| 2026-09-06 | Feature | **Category Visible/Hidden sections + responsive IconPicker** — `app/categories.tsx` partitions filtered `All/Income/Expense` (`FILTERS` Chips retained) into Visible (`hidden=false`) + Hidden (`hidden=true`) vertical sections (In Use / Not Used reference): Visible header `Visible (N)` + `(Long press to reorder)` copy (Phase 1 no drag), Hidden header `Hidden (N)` + owner-only `Delete hidden categories` (`Alert` confirm → `Promise.allSettled` sequential `remove` + `Snackbar` aggregation) ; cards badge `-` (`#FACC15` yellow 18px `C.cardBorder` 2) on Visible top-left `→ hide`, badge `+` yellow + `x` (`#FCA5A5` red) on Hidden `→ unhide/delete` (`Feather` 10px, `hitSlop 6`) reusing `api.categories.update/remove`; `modules/icon-registry` `IconPicker` fix 4-col stretched screenshot → responsive `cols=max(4,min(6,floor((width-40+8)/64)))` via `useWindowDimensions` (56px circle kept, `gap 8`, `getItemLayout` `64*floor(index/cols)`, 360→5, 390→6). Updates §1, §2.1, §3.5, §3.9, §8. Spec `docs/superpowers/specs/2026-09-06-category-visible-hidden-design.md`. Verified `npx tsc --noEmit` + `npm run lint` + `npm test` (no schema, no `convex codegen`). |
