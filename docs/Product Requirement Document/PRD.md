...
> Last updated: 2026-09-09 (Account types → Asset/Debt + required sub-type with fixed per-sub-type icons, ASSET/DEBT sections; prev 2026-09-08 Transaction Form Redesign — UI refresh: `KeyboardAwareScrollView` space-between, CategoryGrid label outside box, Keypad paired operators, bottom card note+amount+chips, pill overlap, keyboard avoidance; prev 2026-09-08 Add Transaction Sheet spec + 2026-09-07 Impeccable typeset audit 1A–7A — heading 700 bold, button 16 keep with docs exception, token explicit, tracking 1.0/0.08em, label 600, subheading 500, full docs sync; tailwind scale baked)
