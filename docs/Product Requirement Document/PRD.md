...
> Last updated: 2026-09-10 Multi-household hardening (delete/leave → /home, targeted listActive, onboarding anti-resubmit); prev 2026-09-10 Settings household list + detail; prev 2026-09-10 Multi-household per user (server-side active); prev 2026-09-09 (Account types → Asset/Debt + required sub-type with fixed per-sub-type icons, ASSET/DEBT sections; prev 2026-09-08 Transaction Form Redesign — UI refresh: `KeyboardAwareScrollView` space-between, CategoryGrid label outside box, Keypad paired operators, bottom card note+amount+chips, pill overlap, keyboard avoidance; prev 2026-09-08 Add Transaction Sheet spec + 2026-09-07 Impeccable typeset audit 1A–7A — heading 700 bold, button 16 keep with docs exception, token explicit, tracking 1.0/0.08em, label 600, subheading 500, full docs sync; tailwind scale baked)
...
| Household | Create (first user becomes Owner), get active household, rename (Owner only), list members, remove member (Owner only; owner cannot be removed), delete/leave/transfer — Owner `households.deleteHousehold` hard-deletes cascade (`households` + `householdMemberships` + `accounts` + `categories` + `transactions` + `budgets` + `periodBalances` + `invitations` + memberships), Member `households.leaveHousehold` removes own membership only (transactions/budgets remain), Owner `households.transferOwnership` swaps `owner↔member` roles. Settings shows a SINGLE Households list (tap row → `/members?householdId=` detail; inline Set-active button + detail "Set as Active" button; Home header switcher unchanged; `HouseholdSwitcher` sheet no longer used in Settings). Danger Zone (delete/leave) moved from Settings into the household detail screen (`isOwner ? Delete Household : Leave Household` + transfer picker via Alert). Preference `periodType` (`monthly` default, extensible `weekly`/`yearly` — `weekly`/`yearly` coming soon via `households.updatePeriodType`) and `balanceMode` (`fresh` default vs `carryOver`) — Owner-only mutations `households.updateBalanceMode`/`updatePeriodType` (validated via `validateBalanceMode`/`validatePeriodType`, patch `households` + recompute cascade `periodBalances`). |
...
| Invitations | Owner generates invite code (8 alphanumeric chars, HMAC-SHA-256 hash stored, 7-day expiry, single-use). Owner can revoke. Generating a new code atomically auto-revokes all previous active codes within the same mutation (`invitations.create` patches `revoked=true` before insert, so at most one active code exists). Member joins by redeeming a code. Rate limited: max 5 attempts/code/min. Multi-household (2026-09-10): redeem is allowed while a member elsewhere — rejected only when already a member of THAT household (`You are already a member of this household.`); sets active only for a first-ever membership; returns `{ householdId }` so the client can show the Stay/Switch dialog. |
| Accounts | Create (optional opening balance → auto "Initial Balance" transaction atomically in the same mutation: reserved category validated before `accounts.insert`, account `balance` set directly to `openingBalance`, transaction inserted with same `now` timestamp), edit (name/type/subType/hidden), delete (guarded if referenced by transactions), list (visibility-filtered, grouped into ASSET/DEBT sections with subtotals). Owner-only management. Verify/reconcile: Owner can verify stored vs expected balances and recalculate all balances from transaction history (atomic per-account patch) when drift is detected (`accounts.verify` / `accounts.reconcile`, 10k tx cap, banner + Recalculate in Accounts tab). Type is `asset` \| `debt` (labels Asset/Debt) with a required validated `subType` (asset → `cash`/`bank`/`ewallet`/`other`, debt → `credit_card`/`other`; server rejects mismatched pairs on create/update). Icon fixed per sub-type via Streamline Ultimate Color (CC BY 4.0) — cash cash-payment-bill, bank saving-bank-1, ewallet wireless-payment-credit-card-dollar, credit_card credit-card-1, other tags-1 — rendered offline via AccountIcon SvgXml 24x24 palette:true, derive at render (icon is display-only, never stored). |
...
**Period handling (as of 2026-08-31):** each household stores `periodType` (`monthly` default, `weekly`/`yearly` extensible — `validatePeriodType`, `PERIOD_TYPES` in `constants/validation.ts` and `utils/period.ts` `PeriodType`) and `balanceMode` (`fresh` default vs `carryOver` — `validateBalanceMode`, `BALANCE_MODES`). `fresh`: each period's `openingBalance 0`, `closingBalance = net (income-expense)` isolated; `carryOver`: `openingBalance = prev closing`, `closingBalance = opening + net` cumulative (cascade from `household.createdAt` to now). Both are Owner-only via `households.updateBalanceMode`/`updatePeriodType` (requireOwner, patch `households` + trigger `periodBalances.recomputeAllForHousehold`). `updatePeriodType` currently throws `Weekly/yearly coming soon` if not `monthly` (extensible via `utils/period.ts` `getWeekBounds`/`getYearBounds`/`getPeriodBounds`). Period boundaries use `utils/period.ts` `getPeriodBounds(ts,tz,type)` — `monthly` via `getMonthBounds` (tz-aware), `weekly` Monday 00:00 tz, `yearly` Jan 1 00:00 tz — with `getPrevPeriod`/`getNextPeriod`/`formatPeriodLabel`/`buildPeriodWindow`. household detail screen (`app/members.tsx?householdId=`) exposes a segmented control (Fresh / Carry Over) Owner-only (`BALANCE_MODE_OPTIONS`, `useMutation` + `hapticSuccess` + `Snackbar`, Member read-only badge; moved from Settings as of 2026-09-10). Snapshot table `periodBalances` (see §3.10, §6) persists per-period aggregates for O(1) Home/Analytics reads.

**Delete / Leave / Transfer (as of 2026-09-04):** Owner `households.deleteHousehold({householdId})` (requireOwner, householdId match) hard-deletes cascade via `cascadeDelete` — `transactions` → `budgets` → `periodBalances` (filter scan) → `accounts` → `categories` → `invitations` → `householdMemberships` → `households`; all members lose access to it (their other households are unaffected). Member `households.leaveHousehold({householdId})` (member-only, owner blocked `Owners cannot leave. Transfer ownership or delete the household.`) deletes own `householdMemberships` only, transactions/budgets remain. Owner `households.transferOwnership({householdId, newOwnerUserId})` validates target is member of same household, swaps roles (`owner→member`, `member→owner`). UI Danger Zone in `app/members.tsx` (moved from Settings as of 2026-09-10) (`Shadow.card` `border #FCA5A5` + `alert-triangle` `C.error`, role-aware `Delete Household` vs `Leave Household` `variant="danger"` + subtitle, `hapticSuccess`/`hapticError` + `Snackbar` + `router.replace("/home")` — Home's existing null effect routes to onboarding only when no households remain); owner flow offers Transfer picker (Alert list of non-owner members → `transferOwnership`) or Delete with double confirm. Verified by `tests/households.deleteLeaveTransfer.test.ts` 8 cases. Multi-household (2026-09-10): `households.listMine` returns all memberships oldest-first with `{household, role, isActive}` (active = `users.activeHouseholdId`, read-time fallback to oldest when unset); `households.switchActive({householdId})` requires membership in that household (any role); `households.create` sets active only for the first-ever household (later creates keep the current active); `leaveHousehold`/`deleteHousehold` reassign the caller's active to the oldest remaining household (or clear when none) — only when the left/deleted one was active.
...
Accounts are where money lives, classified as `asset` (label Asset) or `debt` (label Debt). Each
has an auto-maintained balance plus a required `subType` that only controls the icon: asset → `cash`/`bank`/`ewallet`/`other`, debt → `credit_card`/`other` (validated as a pair with `type` on create/update — `tests/accounts.create.test.ts` pair-rejection + update cases). Owner creates accounts with an optional opening
...
name/type/subType and toggles visibility. Owner
deletes accounts only when no transactions reference them. The Accounts tab groups rows into ASSET and DEBT sections, each with a subtotal; the hero keeps the grand total + Asset/Debt breakdown; the filter rail is All/Asset/Debt.

Icons are fixed per sub-type via ACCOUNT_STREAMLINE_MAP + AccountIcon (`subType` prop, SvgXml offline streamlineIconData.json: cash cash-payment-bill, bank saving-bank-1, ewallet wireless-payment-credit-card-dollar, credit_card credit-card-1, other tags-1), displayed in VaultCard 56/32, Home 40/28, SelectField 24, account-form 24 preview — no Feather tinted circles, neutral C.surface container.
...
| Icons | Feather (UI, 24px) + Streamline Ultimate Color 998 via Iconify (CC BY 4.0, palette:true, 24×24) rendered as SVG via `react-native-svg` `SvgXml` for category icons (56 allowlist → `CATEGORY_STREAMLINE_MAP`, offline `streamlineIconData.json`) + AccountIcon 5 fixed per sub-type (ACCOUNT_STREAMLINE_MAP: cash cash-payment-bill, bank saving-bank-1, ewallet wireless-payment-credit-card-dollar, credit_card credit-card-1, other tags-1, SvgXml offline same bundle, 56/32/40/28/24/20) + png legacy fallback |
...
| Icons | `@expo/vector-icons` Feather (UI) + Streamline Ultimate Color via Iconify (`icon-sets.iconify.design/streamline-ultimate-color`, CC BY 4.0, 998 icons, `react-native-svg` `SvgXml` offline via `constants/streamlineIconData.json` + `components/CategoryIcon.tsx` + `components/AccountIcon.tsx` 5 fixed per sub-type: ACCOUNT_STREAMLINE_MAP cash cash-payment-bill, bank saving-bank-1, ewallet wireless-payment-credit-card-dollar, credit_card credit-card-1, other tags-1, shares same bundle, SvgXml 24x24 palette:true — AccountIcon 56/32/40/28/24/20) |
...
|------|------|-------------|
| 2026-09-10 | Fix | **Multi-household hardening** — delete/leave success navigates to `/home` (Settings + Members; Home's null effect routes to onboarding only when no households remain); onboarding add-mode guards resubmission after create/join (`completed` flag disables submit) + Stay/Switch dialog is non-dismissable with a dedicated Retry-switch action (second failure lands on `/home`, household safe in switcher); Settings content in `ScrollView` (unbounded list can't push sections off viewport); switcher list bounded (`maxHeight 320` scroll); `invitations.listActive` resolves membership in the requested `householdId` so non-active detail screens see their own invites (regression test: active-H1 querying H2). Verified `tsc` + `lint` clean, 190 tests pass. Updates §3.2/§3.3/§6. |
| 2026-09-10 | Feature | **Settings household list + detail** — Settings shows a SINGLE Households list (tap row → `/members?householdId=` detail; inline Set-active button + detail "Set as Active" button; Home header switcher unchanged; `HouseholdSwitcher` sheet no longer used in Settings); Danger Zone (delete/leave) and balance-mode toggle moved from Settings into the household detail screen; `invitations.create({ householdId? })` targets any owned household (defaults to active), `revoke` derives household from the invitation doc so non-active invites are revocable. Verified by `tests/invitations.household.test.ts` (+ `tests/households.multi.test.ts`). Updates §2.1/§3.2/§3.10/§4.5/§5.2. |
| 2026-09-10 | Feature | **Multi-household per user (server-side active)** — `users.activeHouseholdId` (optional, follows across devices) with active resolution in `convex/helpers.ts` (`resolveActive`: stored id when still a member, else read-time fallback to oldest); new `households.listMine` (query, all memberships oldest-first + role + `isActive`) and `households.switchActive` (mutation, requires membership); `households.create` sets active only for the first-ever household; `invitations.redeem` allowed while a member elsewhere (rejected only when already a member of THAT household), sets active only when first-ever, returns `{ householdId }`; `leaveHousehold`/`deleteHousehold` reassign caller active to oldest remaining (or clear when none) only when the left/deleted one was active; UI tappable Home header (chevron when >1 household) + Settings → Households switcher + onboarding `add=1` Stay/Switch dialog (no silent auto-switch). Verified by `tests/households.multi.test.ts`. Updates §1/§2.1/§2.3/§6/Appendix A. |
| 2026-09-09 | Feature | **Account types → Asset/Debt + required sub-type** — `type` shrinks `cash/bank/ewallet/credit_card` → `asset/debt` (labels Asset/Debt); new required `subType` (asset → `cash/bank/ewallet/other`, debt → `credit_card/other`) validated as a pair on create/update (`Sub-type is not valid for this account type.`, incl. joint type+subType updates); icon re-keyed per sub-type (`ACCOUNT_STREAMLINE_MAP` 5 fixed: cash cash-payment-bill, bank saving-bank-1, ewallet wireless-payment-credit-card-dollar, credit_card credit-card-1, other tags-1; `AccountIcon({subType})`); Accounts tab lists ASSET/DEBT sections with subtotals (hero keeps grand total + breakdown, rail All/Asset/Debt); form has sub-type chips resetting on type change (kept when re-tapping active type). Dev data migrated twice via uncommitted one-off mutations (legacy types → asset/debt, then name-inferred sub-types BCA→bank/Dompet→cash/GoPay→ewallet/BSI→bank; shim-push pattern both times, files deleted). Verified `tsc` + `lint` clean, 172 tests pass, CodeRabbit clean. Updates §2.1/§2.2/§3.4/§3.9/§6/§7/§8. |
