...
> Last updated: 2026-08-31 (Period handling — swipeable Home + refined Period Balance + history/timezone & review batch fixes)
...
- **Analytics (below Budgets, as of 2026-08-31 — fixes 2026-08-31 + period snapshot 2026-08-31, cashflow removed 2026-08-31):**
  - **Delta Card** (`components/charts/DeltaCard.tsx`): `GradientCard` showing **closing delta** `currentClosing` ( `balances.closingBalance` for `selectedPeriodStart`) vs `prevClosing` (`prevBalances.closingBalance`) — derived from `periodBalances.get` via `calcDelta(current, prev, periodNoun)` (`utils/analytics.ts` `periodNoun` `week`/`month`/`year` from `periodType`, so weekly `New this week` / `vs last week`, yearly `vs last year`); `prevClosing===0` → period-appropriate `New this week/month/year` / `No change`; **no chip/shape** — right side plain `Feather trending-up/down/minus` + `Text label` `color C.success/C.error/C.textSecondary` `withSpring`. In `fresh` `closing=net`, `carryOver` cumulative — delta reflects `balanceMode` + `periodType` (`DeltaCard` prop `periodType`).
...
- **Recompute cascade:** `convex/periodBalances.ts:114` `recomputeAllForHousehold` — explicit bounded `by_household_date` scan `lt now` `take(10001)` (if `>10000` throw `ConvexError Too many transactions`, prevents unbounded `collect`); no `gte household.createdAt` so history before creation (Aug household Juli) included; hidden-category excluded for `isOwner=false` via `hiddenCache` (`ctx.db.get(categoryId)`), `fresh`/`carryOver` via `computeOpeningClosing`; ordered `firstStart` = 12-month window + `earliestTxStart` (cap 2y) via `getNextPeriod` guard 500, upsert + delete obsolete (`!withBalances.has(periodStart)` → `delete` + `fixed++`). `recomputeFromForHousehold` delegates to full recompute. Fallback in `get`/`listWindow` with `timezone` param (Home `resolveTimezone`) computes on-the-fly; `get` non-owner absent `compM` → `null` not owner `snap` (prevents leak). Triggers: `households.updateBalanceMode`/`updatePeriodType`, `transactions.create/update/delete`, `accounts.create` (`recomputeAllForHousehold`), plus `recomputeAll`/`reconcile`/`backfill` owner mutations. `verify` compares stored vs expected + obsolete deletions.
- **Queries:** `periodBalances.get({periodStart, periodType?, timezone?})` → snapshot or virtual computed fallback; non-owner absent `compM` → `null` not owner `snap` (explicit `return null` at `259`, prevents leak); `periodBalances.listWindow({startDate,endDate,periodType?, timezone?})` → `{balances, isOwner}` filtered sorted, fallback computed if empty + member visibility-scoped recompute (`isOwner=false` with `hiddenCache`); `verify`/`reconcile`/`backfill`/`recomputeAll` as above. `utils/period.ts` `validatePeriodType`/`validateBalanceMode` guard inputs; `constants/validation.ts` `PERIOD_TYPES`/`BALANCE_MODES` shared.
