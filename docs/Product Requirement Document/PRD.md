...
> Last updated: 2026-08-31 (Period handling — swipeable Home + refined Period Balance + history & cashflow fixes)
...
- **Analytics (below Budgets, as of 2026-08-31 — fixes 2026-08-31 + period snapshot 2026-08-31, cashflow removed 2026-08-31):**
  ...
  - All cards use `Shadow.card`, `Radius.md`, `useThemeColors()` (dark variants), `Feather` icons. Loading: `Skeleton` 80/140 (Delta + Donut). Hidden category excluded for Members (periodBalances snapshot excludes transfers, aggregates income/expense signed; spending cached `hiddenCategoryCache`). Offline: shows cached Convex data + `ConnectivityBanner`. Cashflow bar chart removed from Home (was 6-period `periodBalances.listWindow`) as of 2026-08-31 — backend `transactions.cashflow` remains but not rendered in Home.
...
- **Recompute cascade:** `convex/periodBalances.ts:114` `recomputeAllForHousehold(ctx, household, now)` — single `by_household_date` scan `lt now` `take(10001)` (cap 10k, else `Too many transactions`, no `gte household.createdAt` so history before household creation — e.g., Aug household input Juli for analysis — is included), group by `getPeriodBounds(tx.date, timezone, periodType).start` (income sum, expense sum `Math.abs`), build ordered `firstStart..currentStart` via `getNextPeriod` (guard 500, 12 periods back + earliestTxStart), compute opening/closing, upsert (`by_household_period` unique). `recomputeFromForHousehold` delegates to full recompute for `carryOver` correctness. Fallback in `get`/`listWindow` computes on-the-fly with `timezone` param (passed from Home `resolveTimezone`) so July swipe shows actual before materialization. Triggers: `households.updateBalanceMode`/`updatePeriodType` after patch, `transactions.create/update/delete` (`convex/transactions.ts:135`, 9xx) `recomputeAllForHousehold`, `accounts.create` (`convex/accounts.ts:36`) after opening transaction, `periodBalances.recomputeAll`/`reconcile`/`backfill` owner mutations. `verify` compares stored vs expected.
- **Queries:** `periodBalances.get({periodStart, periodType?, timezone?})` → snapshot or virtual computed fallback (member-safe); `periodBalances.listWindow({startDate,endDate,periodType?, timezone?})` → `{balances, isOwner}` filtered sorted, fallback computed if empty; `verify`/`reconcile`/`backfill`/`recomputeAll` as above. `utils/period.ts` `validatePeriodType`/`validateBalanceMode` guard inputs; `constants/validation.ts` `PERIOD_TYPES`/`BALANCE_MODES` shared client/server.
