...
5. **Amount + Keypad** — amount besar kanan + `IDR` label kecil (Q5 — hanya hint, tidak format currency). Input display = `formatAmountInput` live + evaluasi ekspresi. Keypad custom 4×4: `1-9`, `0`, `00?`, `.` (trigger amber warning `wasDecimalTruncated`), `+ - × ÷`, `⌫`, `Today`, `✓` (kuning `C.primary`/`#FACC15` mapped to `C.primary`). Evaluasi: parse `amountExpr` → token number/operator → hitung `left-to-right` (tanpa precedence) atau via `Function` safe? Pilih evaluator manual integer-only, ignore division remainder. On `✓` → `handleSubmit`; on `Today` → open DatePicker.
...
- [ ] Keypad + - × ÷ live thousand, ✓ submit, Today → date picker timezone-aware
- [ ] Note suggest same category, max 200, dark/light
- [ ] Amount bare number + IDR hint, whole-number warning
