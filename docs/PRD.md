...
| Accounts | Create (optional opening balance → auto "Initial Balance" transaction atomically), edit (name/type/subType/hidden), delete (guarded if referenced by transactions), archive/unarchive (reversible; archived hidden from main list + new-transaction pickers, history/balances/reports intact; delete still blocked while transactions reference), list (visibility-filtered, grouped into ASSET/DEBT sections with subtotals plus a collapsed Archived section excluded from main totals). Owner-only management. Verify/reconcile: Owner can verify stored vs expected balances and recalculate when drift is detected. Type is `asset` or `debt` with a required validated `subType`. Icon fixed per sub-type via Streamline Ultimate Color (CC BY 4.0). |
| Categories | Create, edit (name/type/icon/hidden; type change guarded), delete (guarded if referenced), archive/unarchive (reversible; archived hidden from main list + new-transaction pickers, history/reports intact; delete still blocked while transactions/budgets reference), list (visibility-filtered plus a collapsed Archived section). Icon chosen from 56 allowlist rendered via Streamline Ultimate Color 998 icons via Iconify — CC BY 4.0 Streamline. Two reserved "Initial Balance" categories per household are protected. Owner-only management. |
| Transactions | Create/edit/delete income, expense, transfer. Delete offers one-tap Undo that restores the exact transaction (incl. archived account/category references). Account balance(s) auto-update (reverse old, apply new). Transfers move between two accounts, no category. Members respect hidden account/category rules. Archived accounts cannot be used for new transactions or reassignment (owner + member); existing transactions on archived accounts remain editable and reassignable to active accounts. Account filters (Home, Search) include archived accounts so their history stays filterable. Archived categories cannot be used for new transactions or reassignment (owner + member); existing transactions on archived categories remain editable and reassignable to active categories. Category filters (Home, Search) include archived categories so their history stays filterable. `list` returns at most 1 000 rows per page; cursor-paginated. Home ledger is period-bound 30/page with Search+Filter. Search global cross-period with Date first chip, default last 14 days. `summary` query computes range income/expense/net server-side (transfers excluded; Members' hidden-category rows excluded). Supports server-side filtering by transaction type, account, and category. Supports server-side substring search by note, amount string, account name and category name (≥2 chars) — committed only after user taps Search button or submits via keyboard (no auto debounce). Amount input is whole-number only. |
...
| Budgets | Create/edit/delete monthly budgets per expense category. Archived categories cannot be used for new budgets (owner + member); existing budgets on archived categories remain. List for a month with spent/progress. Members can fully manage. Budgets for hidden categories stay visible to Members. Create form shows quick-fill suggestion chips after category selection (last month's budget, last month's spent, adaptive 3-month average spent); tapping a chip fills Amount. |
...
- **Archived Account:** hidden from the main Accounts list and new-transaction pickers for everyone (Owner + Member); transaction history, balances, and reports still include it. Nobody can create on it or reassign to it (`"This account is archived."`); anyone may edit an existing transaction on it or reassign it to an active account. Members see archived accounts in the Archived section only when not hidden. Archiving is reversible; physical delete stays blocked while transactions reference the account.
- **Archived Category:** hidden from the main Categories list and new-transaction/budget pickers for everyone (Owner + Member); transaction history, budgets, and reports still include it. Nobody can create on it or reassign to it (`"This category is archived."`); anyone may edit an existing transaction on it or reassign it to an active category. Members see archived categories in the Archived section only when not hidden (archived+hidden invisible; archived+visible visible-but-unusable). Archiving is reversible; physical delete stays blocked while transactions/budgets reference the category.
...
- Account/category colors; multi-currency; category merging (account + category archiving implemented as of 2026-09-20).
