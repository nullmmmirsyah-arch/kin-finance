...
| `app/(tabs)/home.tsx` | Swipeable dashboard (`react-native-pager-view` paging, 12 periods, `selectedPeriodStart` via `getPeriodBounds`/`buildPeriodWindow`, `<`/`>` + dots `formatPeriodLabel`, `pagerRef` + `hapticSuccess`): household card, **Period Balance** (`GradientCard` refined: `PERIOD BALANCE` pill + closing 28 + `Income/Expense` tinted circles via `periodBalances.get` + `timezone`), total budget jar card per `selectedPeriodStart` (`BudgetTotalCard` + `summarizeBudgets` from `utils/budgets.ts`; tap opens `BudgetBreakdownSheet` bottom sheet with all budgets in Budgets-tab order, pass-through, no slice/sort), Analytics below Budgets — `DeltaCard` (closing delta) + `SpendingDonut` (selected period); `PagerView` + `ScrollView` `RefreshControl` (`refreshKey`), stale derives from `balances === undefined` + `spendingRes === undefined` |
...
| `components/` | Reusable UI (Button, Input, Card, Fab, EmptyState, Snackbar with optional action, Skeleton, ThemeProvider, TransactionCard, Chip, DateField, GradientCard, SelectField with search, ConnectivityBanner, BrandedLoadingShell, UpdateBanner, BudgetTotalCard, BudgetBreakdownSheet) + `transaction/*` picker primitives (`AccountPill` with `subLabel`, `CategoryGrid` with `remainingText` captions, `TransferDual`) + non-UI controllers (OtaUpdater) |
...
| `utils/budgets.ts` | Pure budget aggregation `summarizeBudgets` (mirrors `app/(tabs)/budgets.tsx` total semantics: budgeted/sum, spent skipping redacted, `hasRedacted`, progress, honeyLevel, remaining) — consumed by Home total jar card |
| `utils/remaining.ts` | Pure live-projection helpers `projectAccountBalance` / `projectBudgetRemaining` (current ± typed amount; edit-mode old-amount correction only for the same account/category while the old tx stays in the queried budget period; raw current when amount empty) — consumed by `transaction-form` projections; covered by `tests/remaining.test.ts` |
